...
--Find interviews related to bakery
SELECT transcript FROM interviews
WHERE year = 2024 AND month = 7 AND day = 28 AND transcript LIKE "%bakery%" ;

-- Check bakery's security logs based on date and time
...
-- Update phone calls that match raceiver nunber with someone's number
UPDATE phone_calls
SET receiver = people.name
FROM people
WHERE phone_calls.receiver = people.phone_number;

-- Find caller and reciever with time and date along with duration
...
-- Find destination, number, and license number
...
-- Find that thief who stole the duck by name in common of all recent queries
SELECT people.name
